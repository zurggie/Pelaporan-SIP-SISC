...
$max = $auth_user->runQuery("SELECT MAX(S421) as maxs421,MAX(S411) as maxs411,MAX(S422) as maxs422,MAX(S431) as maxs431,MAX(S441) as maxs441,MAX(S442) as maxs442,MAX(S451) as maxs451,MAX(S461) as maxs461 
FROM sisc_guru_data WHERE NOKP=:nokp");
$max->bindParam(':nokp',$nokp);    
$max->execute();
$maxRow=$max->fetch(PDO::FETCH_ASSOC);
$maxs421=$maxRow['maxs421']*10/4;
$maxs411=$maxRow['maxs411']*10/4;
$maxs422=$maxRow['maxs422']*5/4;    
$maxs431=$maxRow['maxs431']*15/4; 
$maxs441=$maxRow['maxs441']*25/4;    
$maxs442=$maxRow['maxs442']*5/4; 
$maxs451=$maxRow['maxs451']*10/4;    
$maxs461=$maxRow['maxs461']*20/4;
...
<button class="accordion">
    ...
    <?php echo "$nokp |  $nama |"; ?></button>
<div class="panel">

   <table border="1" width="100%">
    <tr><td> TOV</td><td><? $tov;?></td></tr>   
    </table>

    <table border="1" width="100%">
        <tr><td colspan="9"> <center><b>Standerd</b></center></td></tr>
...
?>
    ...
    <tr>
        ...
        <? if($jenis=="KELOMPOK"){
        ?>
        <td colspan="8"> <center><b>PLC </b>  </center></td>
      <?      
    }
    else{
...
$result->execute();
    ...
    ?>                          
                            ...
                            <h3 class="box-title m-b-0">Analisa Jenis Kekerpan Guru Yang Dibimbing</h3>
                            <p class="text-muted m-b-30">Eksport data Ke Salin, CSV, Excel, PDF & Print</p>
                            <div class="table-responsive">
                                <table id="example23" class="display nowrap" cellspacing="0" width="100%">
                                    <thead>
...
foreach ($result as $row) {
    ...
    $sqln = $auth_user->runQuery("SELECT * FROM sisc_guru_data  WHERE NOKP=:nokp AND BILKE=1");
	$sqln->execute(array(":nokp"=>$nokp));	
    $sqlnRow=$sqln->fetch(PDO::FETCH_ASSOC);
    $id1=$sqlnRow['ID'];   
    $jenisbimbingan=$sqlnRow['JENISBIMBINGAN'];
    ...
    $tov= round((($s411+$s421+$s431+$s441+$s442+$s451+$s461+$s422)/800*100),2);
    
      
$sqln = $auth_user->runQuery("SELECT * FROM sisc_guru_data  WHERE NOKP=:nokp AND BILKE=2");
	$sqln->execute(array(":nokp"=>$nokp));	
    $sqlnRow=$sqln->fetch(PDO::FETCH_ASSOC);
    $id2=$sqlnRow['ID'];   
    $jenisbimbingan2=$sqlnRow['JENISBIMBINGAN']; 
    $s411=$sqlnRow['S411']; 
    $s421=$sqlnRow['S421'];
    $s422=$sqlnRow['S422'];
    $s431=$sqlnRow['S431'];
    ...
    $ar2= round((($s411+$s421+$s431+$s441+$s442+$s451+$s461+$s422)/800*100),2);
    
      
    
$sqln = $auth_user->runQuery("SELECT * FROM sisc_guru_data  WHERE NOKP=:nokp AND BILKE=3");
	$sqln->execute(array(":nokp"=>$nokp));	
    $sqlnRow=$sqln->fetch(PDO::FETCH_ASSOC);
    $id2=$sqlnRow['ID'];   
    $jenisbimbingan2=$sqlnRow['JENISBIMBINGAN']; 
    ...
    $ar3= round((($s411+$s421+$s431+$s441+$s442+$s451+$s461+$s422)/800*100),2);
    
      
  
$sqln = $auth_user->runQuery("SELECT * FROM sisc_guru_data  WHERE NOKP=:nokp AND BILKE=4");
	$sqln->execute(array(":nokp"=>$nokp));	
    $sqlnRow=$sqln->fetch(PDO::FETCH_ASSOC);
    $id2=$sqlnRow['ID'];   
    $jenisbimbingan2=$sqlnRow['JENISBIMBINGAN']; 
    ...
    $ar4= round((($s411+$s421+$s431+$s441+$s442+$s451+$s461+$s422)/800*100),2);
    
  
$sqln = $auth_user->runQuery("SELECT * FROM sisc_guru_data  WHERE NOKP=:nokp AND BILKE=5");
	$sqln->execute(array(":nokp"=>$nokp));	
    $sqlnRow=$sqln->fetch(PDO::FETCH_ASSOC);
    $id2=$sqlnRow['ID'];   
    $jenisbimbingan2=$sqlnRow['JENISBIMBINGAN']; 
    ...
    $ar5= round((($s411+$s421+$s431+$s441+$s442+$s451+$s461+$s422)/800*100),2);
    
  
$sqln = $auth_user->runQuery("SELECT * FROM sisc_guru_data  WHERE NOKP=:nokp AND BILKE=6");
	$sqln->execute(array(":nokp"=>$nokp));	
    $sqlnRow=$sqln->fetch(PDO::FETCH_ASSOC);
    $id2=$sqlnRow['ID'];   
    $jenisbimbingan2=$sqlnRow['JENISBIMBINGAN']; 
    ...
    $ar6= round((($s411+$s421+$s431+$s441+$s442+$s451+$s461+$s422)/800*100),2);
    
    
    
  
$sqln = $auth_user->runQuery("SELECT * FROM sisc_guru_data  WHERE NOKP=:nokp AND BILKE=7");
	$sqln->execute(array(":nokp"=>$nokp));	
    $sqlnRow=$sqln->fetch(PDO::FETCH_ASSOC);
    $id2=$sqlnRow['ID'];   
    $jenisbimbingan2=$sqlnRow['JENISBIMBINGAN']; 
    ...
    $ar7= round((($s411+$s421+$s431+$s441+$s442+$s451+$s461+$s422)/800*100),2);
    
    
  
$sqln = $auth_user->runQuery("SELECT * FROM sisc_guru_data  WHERE NOKP=:nokp AND BILKE=8");
	$sqln->execute(array(":nokp"=>$nokp));	
    $sqlnRow=$sqln->fetch(PDO::FETCH_ASSOC);
    $id2=$sqlnRow['ID'];   
    $jenisbimbingan2=$sqlnRow['JENISBIMBINGAN']; 
    ...
    $ar8= round((($s411+$s421+$s431+$s441+$s442+$s451+$s461+$s422)/800*100),2);
    
    
  
$sqln = $auth_user->runQuery("SELECT * FROM sisc_guru_data  WHERE NOKP=:nokp AND BILKE=9");
	$sqln->execute(array(":nokp"=>$nokp));	
    $sqlnRow=$sqln->fetch(PDO::FETCH_ASSOC);
    $id2=$sqlnRow['ID'];   
    $jenisbimbingan2=$sqlnRow['JENISBIMBINGAN']; 
    ...
    $ar9= round((($s411+$s421+$s431+$s441+$s442+$s451+$s461+$s422)/800*100),2);
    
    
  
$sqln = $auth_user->runQuery("SELECT * FROM sisc_guru_data  WHERE NOKP=:nokp AND BILKE=10");
	$sqln->execute(array(":nokp"=>$nokp));	
    $sqlnRow=$sqln->fetch(PDO::FETCH_ASSOC);
    $id2=$sqlnRow['ID'];   
    $jenisbimbingan2=$sqlnRow['JENISBIMBINGAN']; 
    ...
    $ar10= round((($s411+$s421+$s431+$s441+$s442+$s451+$s461+$s422)/800*100),2);
 ...
 <a href=\"../modulsisc/pelaporan.php?id=$id\" title=\"Pepaloran[ $id ]$nama \"> 
  ...
  if($tov > 1){
   echo "<b>$tov</b>";
       ...
       if($ar2 > 0 and $ar2 < $tov ) {
   echo"
   <span class=\"label label-danger\">$ar2</span>";
   } 
    
     if( $ar2 > 0 and $ar2 == $tov ){
    echo "<span class=\"label label-warning\"> $ar2</span>";
    
    }  
    
    if( $ar2 > 0 and $ar2 > $tov) {
   echo"<span class=\"label label-success\">$ar2</span>";
   }  
  ...
  <?php
       ...
       if($ar3 > 0 and $ar3 < $tov ) {
   echo"
   <span class=\"label label-danger\">$ar3</span>";
   } 
    
     if( $ar3 > 0 and $ar3 == $tov ){
    echo "<span class=\"label label-warning\"> $ar3</span>";
    
    }  
    
    if( $ar3 > 0 and $ar3 > $tov) {
   echo"<span class=\"label label-success\">$ar3</span>";
   }  
    ...
    <?php                                               
      ...
      if($ar4 > 0 and $ar4 < $tov ) {
   echo"
   <span class=\"label label-danger\">$ar4</span>";
   } 
    
     if( $ar4 > 0 and $ar4 == $tov ){
    echo "<span class=\"label label-warning\"> $ar4</span>";
    
    }  
    
    if( $ar4 > 0 and $ar4 > $tov) {
   echo"<span class=\"label label-success\">$ar4</span>";
   }  
                                            ...
                                            <td>
     <?php                                               
   if($ar5 > 0 and $ar5 < $tov ) {
   echo"
   <span class=\"label label-danger\">$ar5</span>";
   } 
    
     if( $ar5 > 0 and $ar5 == $tov ){
    echo "<span class=\"label label-warning\"> $ar5</span>";
    
    }  
    
    if( $ar5 > 0 and $ar5 > $tov) {
   echo"<span class=\"label label-success\">$ar5</span>";
   }  
                                             ...
                                             <td>
   <?php                                               
  if($ar6 > 0 and $ar6 < $tov ) {
   echo"
   <span class=\"label label-danger\">$ar6</span>";
   } 
    
     if( $ar6 > 0 and $ar6 == $tov ){
    echo "<span class=\"label label-warning\"> $ar6</span>";
    
    }  
    
    if( $ar6 > 0 and $ar6 > $tov) {
   echo"<span class=\"label label-success\">$ar6</span>";
   }  
     ?>                                          
                                             ...
                                             <td>
   <?php                                               
   if($ar7 > 0 and $ar7 < $tov ) {
   echo"
   <span class=\"label label-danger\">$ar7</span>";
   } 
    
     if( $ar7 > 0 and $ar7 == $tov ){
    echo "<span class=\"label label-warning\"> $ar7</span>";
    
    }  
    
    if( $ar7 > 0 and $ar6 > $tov) {
   echo"<span class=\"label label-success\">$ar7</span>";
   }  ?>                                           
  ...
  if($ar9 > 0 and $ar9 < $tov ) {
   ...
   <span class=\"label label-danger\">$ar6</span>";
    ...
    if( $ar10 > 0 and $ar10 > $tov) {
   echo"<span class=\"label label-success\">$ar10</span>";
   } ?>                                         
